...
class DataBase:
    ...
    def __init__(self, config_file='configs/database.yml') -> None:
        ...
        self._connection_pool = pool.SimpleConnectionPool(minconn=1, maxconn=10, dbname=config['postgres']['dbname'],
                                               user=config['postgres']['user'],
                                               password=config['postgres']['password'],
                                               host=config['postgres']['host'],
                                               port=config['postgres']['port'])
        ...
        self.unavailable_trips_statistics_table = UnavailableTripsStatisticsTable(self._connection_pool)
        self.available_trips_statistics_table = AvailableTripsStatisticsTable(self._connection_pool)
